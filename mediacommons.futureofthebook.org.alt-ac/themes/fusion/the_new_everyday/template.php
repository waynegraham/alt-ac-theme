...
function the_new_everyday_theme() {
  return array(
    // The form ID.
   ...
   */
  function the_new_everyday_search_block_form($form) {
...
function the_new_everyday_preprocess_page(&$vars) {
...
function the_new_everyday_preprocess_comment_wrapper(&$vars) {
...
function the_new_everyday_preprocess_search_result( &$vars ) {
  ...
  if ( $result_node->type == 'contributed_pieces' ) {
    ...
    $div .= '<div class="user-picture"><img src="' . the_new_everyday_get_user_avatar($result_node->uid) . '" /></div>';
    $div .= '</div>';
  }
...
function the_new_everyday_get_user_avatar($uid) {
  global $base_url;
...
function the_new_everyday_preprocess_node(&$vars) {
  ...
  if ( $vars['node']->type == 'cluster' ) {
    ...
    if ( !empty( $vars['node']->field_contributed_piece ) ) {
      ...
      foreach ( $vars['node']->field_contributed_piece as $piece ) {
        ...
        $vars['pieces'] .= the_new_everyday_piece_div( $piece_node, 'cluster-page' );
      }
      $vars['pieces'] .= '</div>';
    }
...
function the_new_everyday_piece_div( $piece_node, $location ) {
  $div = '';
  $piece_account = user_load( array( 'uid' => $piece_node->uid ) );
	$authors_string = tne_contributed_by( l($piece_account->realname, 'http://mediacommons.futureofthebook.org/user/'. $piece_node->uid , array('attributes' => array('title' => t($piece_account->realname), 'class' => 'username'))), $piece_node->field_additional_authors );
...
function the_new_everyday_realname($key) {
...
function the_new_everyday_taxonomy_links($node, $vid) {
...
function tne_contributed_by( $first_author_name, $additional_authors ) {
...
function tne_contributed_by_title( $first_author_name, $first_author_id, $additional_authors ) {
	$all_authors = $first_author_name.' '.tne_description($first_author_id);
	if ( !empty( $additional_authors[0]['uid'] ) ) {
	 $all_authors .= ',<br />';
...
function tne_description($author_id) {
...
function tne_contributed_by_photos($first_author, $additional_authors ) {
...
function tne_cluster_navigator($parent_node_id, $current_piece_id) {
